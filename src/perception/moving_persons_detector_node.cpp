...
void detect_motion() {
    ...
    for (int loop=0; loop<nb_beams; loop++ )//loop over all the hits
    {
        ...
        else dynamic[loop] = 0;//else its static
	}
...
//else we start a new cluster with the current hit and end the current cluster
	...
	int loop;
    for( loop=1; loop<nb_beams; loop++ )//loop over all the hits
    {
        ...
        if(distancePoints(current_scan[loop-1],current_scan[loop]) < cluster_threshold)
        {
			...
			if (dynamic[loop]) nb_dynamic++;
		}
        else
        {
			/*the current hit doesnt belong to the same hit
              1/ we end the current cluster, so we update:
              - cluster-end to store the last hit of the current cluster OK
              - cluster_dynamic to store the percentage of hits of the current cluster that are dynamic PAS CERTAIN
              - cluster_size to store the size of the cluster ie, the distance between the first hit of the cluster and the last one OK
              - cluster_middle to store the middle of the cluster OK */
              
			cluster_end[nb_cluster]=loop-1;
			cluster_size[nb_cluster]=distancePoints(current_scan[cluster_start[nb_cluster]],current_scan[loop-1]);
			cluster_middle[nb_cluster]=geometry_msgs::Point();
			cluster_middle[nb_cluster].x= (current_scan[cluster_start[nb_cluster]].x+current_scan[loop-1].x) / 2.0;
			cluster_middle[nb_cluster].y= (current_scan[cluster_start[nb_cluster]].y+current_scan[loop-1].y) / 2.0;
			cluster_dynamic[nb_cluster]=(nb_dynamic * 100) / (cluster_end[nb_cluster] - cluster_start[nb_cluster] + 1.0);
...
// - more than "dynamic_threshold"% of its hits are dynamic (see, cluster_dynamic table)
    ...
    for (int loop=0; loop<nb_cluster; loop++)//loop over all the clusters
    {
		...
		if(cluster_size[loop] > leg_size_min && cluster_size[loop] < leg_size_max && cluster_dynamic[loop] >= dynamic_threshold)
		{
            ...
            //textual display
            ROS_INFO("jambe qui bouge detected[%i]: cluster[%i]", nb_moving_legs_detected, loop);
		...
		else ROS_INFO("pas de jambe qui bouge dans le cluster[%i]", loop);
	}
    ...
    for (int loop_leg1=0; loop_leg1<nb_moving_legs_detected; loop_leg1++)//loop over all the legs
    {
        for (int loop_leg2=loop_leg1+1; loop_leg2<nb_moving_legs_detected; loop_leg2++)//loop over all the legs
        {
            ...
            if(distancePoints(moving_leg_detected[loop_leg1],moving_leg_detected[loop_leg2]) < legs_distance_max)
            {
				...
				moving_persons_detected[nb_moving_persons_detected] = geometry_msgs::Point();
				moving_persons_detected[nb_moving_persons_detected].x = (moving_leg_detected[loop_leg1].x + moving_leg_detected[loop_leg2].x) / 2.0;
				moving_persons_detected[nb_moving_persons_detected].y = (moving_leg_detected[loop_leg1].y + moving_leg_detected[loop_leg2].y) / 2.0;
				...
				//update of the goal
				pub_moving_persons_detector.publish(moving_persons_detected[nb_moving_persons_detected]);
